SP.model <- function(rho, theta.bar, sig.u, sig.e, sig.n, ord, S, sig.z=0, tol=1e-15){
  ...
  # here s_t = (theta_t^(0), ..., theta_t^(ord), x_(1,t)^(1),...,x_(1,t)^(ord), ..., x_(S,t)^(ord), e_t, theta_{t-1}^(1))
  ...
  aux <- StateSpaceMatLag(ord = ord, rho = rho, sig.u = sig.u, sig.e = sig.e, sig.n = sig.n, tol = tol)
  M <- aux$M
  N <- aux$N
  Q <- aux$Q
  V <- aux$V
  ...
  # drift
  dt <- matrix(0, nrow = (ord + 1 + ((ord)*S) + 2))
  
  # transition matrix
  Tt <- diag(S) %x% Q     # %x% is Kronecker product
  Tt <- cbind(Tt, matrix(0, ncol=2, nrow=(S*ord)))
  Tt <- cbind(matrix(0, ncol=(ord+1), nrow=(S*ord)), Tt)
  Tt <- rbind(Tt, rep(0, ord+1 + ord*S + 2) )  # row of all zero for e_t
  Tt <- rbind(Tt, c(0,1, rep(0, (ord-1) + ord*S + 2) ) )  # unit vector to select theta_{t-1}^(1)
  Tt <- rbind( cbind(M, matrix(0, ncol = ((S*ord) + 2), nrow=(ord+1) )), Tt)
  
  # shock variance matrix HHt = E(Ht eta eta' Ht') = Ht Ht'
  Ht <- diag(S) %x% V
  Ht <- cbind(matrix(0, ncol=2, nrow=(S*ord)), Ht)
  Ht <- rbind(Ht, c(0, 1, rep(0, S)))  # put e_t into current state vector
  ...
  # observation matrix
  Zt <- matrix(0, nrow=(1+S), ncol=(ord+1 + (S*ord) + 2))
  Zt[1,ord+1 + (ord*S) + 1] <- sig.e  # consensus price is theta_{t-1}^(1) + sig.e e_t
  Zt[1,ord+1 + (ord*S) + 2] <- 1  
  Zt[2:(S+1),2] <- 1
  for (s in 1:S){
