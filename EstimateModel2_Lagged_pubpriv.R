estimate.model <- function(paras0, yt, ord, l, h, opt.algo="Nelder-Mead"){
  ...
  objective <- function(paras, yt, ord, l, h, tol=1e-15) {
    ...
    sig.e <- (h + l*exp(-paras[4]))/(1 + exp(-paras[4]))
    sig.n <- (h + l*exp(-paras[5]))/(1 + exp(-paras[5]))
    
    sp <- SP.model(rho = rho, theta.bar = theta.bar, sig.u = sig.u, sig.e = sig.e, sig.n = sig.n, ord = ord, tol = tol , S = S)
    
    a0 <- c(rep(0, ord+1), rep(0,S*ord), c(0,0))
    P0 <- diag(S) %x% sp$SIG
    P0 <- cbind( matrix(0, ncol=(ord+1), nrow=(S*ord)), P0)
    P0 <- rbind( cbind(sp$PP, matrix(0, nrow=(ord+1), ncol=(S*ord))), P0)
    P0 <- cbind(P0, c(rep(sig.e^2,(ord+1)), rep(0,(S*ord))))
    P0 <- cbind(P0, rep(0,(ord+1) + (S*ord)))
    P0 <- rbind(P0, c(rep(sig.e^2,(ord+1)), rep(0,(S*ord)), 1, 0))
    P0 <- rbind(P0, c(rep(0,(ord+1) + (S*ord) + 1), sig.e^2))
  ...
  log.sig.e.0 <- log((paras0[4]-l)/(h-paras0[4]))
  log.sig.n.0 <- log((paras0[5]-l)/(h-paras0[5]))
  paras0.trans <- c(trans.rho.0, theta.bar.0, log.sig.u.0, log.sig.e.0, log.sig.n.0)
  ...
  est <- fit$par
  estimates <- c(rho=1/(1+exp(-est[1])), theta.bar=est[2], sig.u=(h + l*exp(-est[3]))/(1 + exp(-est[3])), sig.e=(h + l*exp(-est[4]))/(1 + exp(-est[4])), sig.n=(h + l*exp(-est[5]))/(1 + exp(-est[5])))
